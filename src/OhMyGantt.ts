...
import { toDate, createTimeList } from './utils/dateHelper'
...
export default class OhMyGantt {
  ...
  constructor(element: Element | string, options: MyGanttOptions) {
    const from = options.from || options.startTime || new Date(new Date().getTime() - 1000 * 60 * 60 * 24 * 7)
    const to = options.to || options.endTime ||  new Date(new Date().getTime() + 1000 * 60 * 60 * 24 * 7)
    ...
    $element.classList.add('omg-gantt')
    ...
    this.from = toDate(opt.from)
    this.to = toDate(opt.to)

    // 初始化日期列表
    this.timeList = createTimeList(this.from, this.to, options.timeInterval)
    this.render()
